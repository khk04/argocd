<template>
  <v-card>
    <v-card-title>작업 수정</v-card-title>
    <v-card-text>
      ...
      <v-radio-group v-model="job_control" class="" inline>
        ...
        <v-radio label="Clous(Azure)" value="hpc_Job_control"></v-radio>
      </v-radio-group>
      <v-select
        label="이미지 선택"
        v-model="selectImage"
    ...
    <v-card-actions>
      ...
      <v-btn color="primary" @click="editJob">수정</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
export default {
    ...
    return {
      ...
      NodeRules: [
        (value) => {
          ...
          return `NODE 설정범위는 1 ~ ${this.max_node} 입니다.`;
        },
      ],
      memRules: [
        (value) => {
  ...
  mounted() {
    ...
    this.selectImage = this.image.replace("docker.juxtagene.com", "");
    ...
    this.job_control = this.hpc_data.job_control;
  },
};
</script>
