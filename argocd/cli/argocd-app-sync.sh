...
kubectl --kubeconfig $KUBE_CONFIG -n ${NAME_SPACE} exec -it $argocd_pod -- bash -c 'argocd login localhost:8080; for app in $(argocd app list -o name); do argocd app sync $app; done'
# kubectl --kubeconfig $KUBE_CONFIG -n ${NAME_SPACE} cp $argocd_pod:/home/backup.yaml backup.yaml
