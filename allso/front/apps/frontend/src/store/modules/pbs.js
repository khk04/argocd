...
const actions = {
    async getPbsNodes() {
        try {
            const response = await axios.get(`/api/pbs/nodes/cluster1`)
    ...
    async getNodeInfo(nodes) {
        ...
        nodes.forEach((node, index) => {
            ...
            // let max = Number(node["tcpus"])
            // let min = 0

            // let random = Math.floor(Math.random() * max + min)
            // rcpus = rcpus + random
            // icpus = icpus + (Number(node["tcpus"]) - random)

            rcpus = rcpus + Number(node['rcpus'])
            icpus = icpus + Number(node['icpus'])

            // let tmem = Number(node.rmem + node.imem)
            // let mmax = Number(node.rmem + node.imem)
            // let mmin = 0

            // let mrandom = Math.floor(Math.random() * mmax + mmin)
            // console.log("mrandom: " + mrandom)

            // rmem = rmem + mrandom
            // imem = imem + (tmem - mrandom)
        ...
        if (rnodes === 0) {
            ...
            noderate = Math.floor((rcpus / tcpus) * 100)
        }
    ...
    async getPbsJobs() {
        try {
            const response = await axios.get(`/api/pbs/jobs/cluster1`)
